...
{
    public class bankniftyModel : PageModel
    {
        private readonly ApplicationBankDbContext _applicationBankDbContext;
        private readonly IHttpClientFactory _httpClientFactory;
        private readonly ILogger<IndexPageModel> _logger;

        public bankniftyModel(ApplicationBankDbContext applicationBankDbContext, IHttpClientFactory httpClientFactory, ILogger<IndexPageModel> logger)
        {
            _applicationBankDbContext = applicationBankDbContext;
            _httpClientFactory = httpClientFactory;
            _logger = logger;
        }


        public List<Company> BankCompanies { get; set; } = new List<Company>();
        ...
        {
            var httpClient = _httpClientFactory.CreateClient("nsebankClient");
            ...
            for (int retry = 0; retry < 10; retry++)
            {
                try
                {
                    var response = await httpClient.GetAsync("https://www.nseindia.com/api/equity-stockIndices?index=NIFTY%20BANK");
            ...
            try
            {
                ...
                BankCompanies = root.Data.Select(d => new Company
                {
                    Symbol = d.Symbol,
                ...
                _applicationBankDbContext.BankCompanies.RemoveRange(_applicationBankDbContext.BankCompanies);
                _applicationBankDbContext.BankCompanies.AddRange(BankCompanies);
                await _applicationBankDbContext.SaveChangesAsync();
            }
            catch (Exception ex)
            {
